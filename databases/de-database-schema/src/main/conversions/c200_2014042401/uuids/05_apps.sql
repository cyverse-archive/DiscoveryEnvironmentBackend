...
UPDATE app_steps SET app_id =
    (SELECT a.id FROM apps a WHERE transformation_task_id_v187 = a.hid_v187);
UPDATE ratings SET app_id =
    (SELECT a.id FROM apps a WHERE transformation_activity_id_v187 = a.hid_v187);
UPDATE app_category_app SET app_id =
    (SELECT a.id FROM apps a WHERE template_id_v187 = a.hid_v187);
UPDATE workflow_io_maps m SET app_id =
    (SELECT a.id FROM apps a
     LEFT JOIN transformation_activity_mappings_v187 tm
     ON tm.transformation_activity_id = a.hid_v187
     WHERE m.hid_v187 = tm.mapping_id);
