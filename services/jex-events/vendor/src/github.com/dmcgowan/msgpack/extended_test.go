...
func TestExtendedSimple(t *testing.T) {
	...
	EncodeSimpleStruct := func(v reflect.Value) (int, []byte, error) {
	...
	extensions.SetEncoder(EncodeSimpleStruct)
	extensions.AddDecoder(7, reflect.TypeOf(&SimpleStruct{}), DecodeSimpleStruct)
...
func TestExtendedInterfaces(t *testing.T) {
	...
	EncodeReader := func(iv reflect.Value) (int, []byte, error) {
		switch v := iv.Interface().(type) {
		case *ReaderWrapper:
			readers = append(readers, v)
			return 17, []byte{byte(len(readers) - 1)}, nil
		case io.Reader:
	...
	extensions := NewExtensions()
	extensions.SetEncoder(EncodeReader)
	extensions.AddDecoder(17, reflect.TypeOf(new(ReaderWrapper)), DecodeReader)
