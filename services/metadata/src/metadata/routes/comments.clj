...
(defroutes* data-comment-routes
  (context* "/filesystem/entry" []
    ...
    (GET* "/:entry-id/comments" [entry-id :as {uri :uri}]
      ...
      (service/trap uri comments/list-data-comments entry-id))

    (POST* "/:entry-id/comments" [user data-type :as {uri :uri}]
      :path-params [entry-id :- TargetIdPathParam]
      ...
      (service/trap uri comments/add-data-comment user entry-id data-type body))

    (PATCH* "/:entry-id/comments/:comment-id" [:as {uri :uri}]
      :path-params [entry-id :- TargetIdPathParam
...
`retracted` parameter to `false`."
      (service/trap uri comments/update-data-retract-status user entry-id comment-id retracted))))

(defroutes* admin-data-comment-routes
  (context* "/admin/filesystem/entry" []
    :tags ["admin-data-comments"]

    (DELETE* "/:entry-id/comments/:comment-id" [:as {uri :uri}]
      :path-params [entry-id :- TargetIdPathParam
                    comment-id :- CommentIdPathParam]
      ...
      (service/trap uri comments/delete-data-comment entry-id comment-id))
...
This endpoint also allows a user to readmit a comment the user previously retracted by setting the
`retracted` parameter to `false`."
      (service/trap uri comments/admin-update-retract-status user entry-id comment-id retracted))))
