INSERT INTO rule_type
    ...
    SELECT 'rt07303d4b-3635-4934-93e6-b24de4f2725b', 'IntBelowField',
           'Needs to be less than another associated parameter', NULL, TRUE, '5',
           'Value must be below: {FieldRef}.', rs.hid
    FROM rule_subtype rs WHERE rs.name = 'Integer';

INSERT INTO rule_type
    (id, name, description, label, deprecated, display_order,
     rule_description_format, rule_subtype_id)
    SELECT 'rt2c3eec11-011a-4152-b27b-00d73deab7cf', 'IntAbove',
           'Has a lower limit (integer)', NULL, FALSE, '3',
           'Value must be above: {Number}.', rs.hid
    FROM rule_subtype rs WHERE rs.name = 'Integer';

INSERT INTO rule_type
    (id, name, description, label, deprecated, display_order,
     rule_description_format, rule_subtype_id)
    SELECT 'rte04fb2c6-d5fd-47e4-ae89-a67390ccb67e', 'IntRange',
           'Has a range of integers allowed', NULL, FALSE, '1',
           'Value must be between: {Number} and {Number}.', rs.hid
    FROM rule_subtype rs WHERE rs.name = 'Integer';

INSERT INTO rule_type
    (id, name, description, label, deprecated, display_order,
     rule_description_format, rule_subtype_id)
    SELECT 'rte1afc242-8962-4f0c-95be-5a6363cdd48b', 'IntAboveField',
           'Needs to be greater than another associated parameter', NULL, TRUE, '6',
           'Value must be above: {FieldRef}.', rs.hid
    FROM rule_subtype rs WHERE rs.name = 'Integer';

INSERT INTO rule_type
    (id, name, description, label, deprecated, display_order,
     rule_description_format, rule_subtype_id)
    SELECT 'rt0621f097-1b31-4457-812b-c8ca70bfbe14', 'MustContain',
           'Must contain certain terms', NULL, TRUE, '7',
           'Value must contain: {List}.', rs.hid
    FROM rule_subtype rs WHERE rs.name = 'Integer';

INSERT INTO rule_type
    (id, name, description, label, deprecated, display_order,
     rule_description_format, rule_subtype_id)
    SELECT 'rt58cd8b75-5598-4490-a9c9-a6d7a8cd09dd', 'DoubleRange',
...
INSERT INTO rule_type
    (id, name, description, label, deprecated, display_order,
     rule_description_format, rule_subtype_id)
    SELECT 'rtaebaaff6-3280-442d-b45e-6fd65e2d2c80', 'IntBelow',
           'Has a higher limit (integer)', NULL, FALSE, '4',
           'Value must be below: {Number}.', rs.hid
    FROM rule_subtype rs WHERE rs.name = 'Integer';
...
INSERT INTO rule_type
    ...
    SELECT '4b4ee99b-2cf2-4ff8-8474-73fc6a1effa7', 'Regex',
           ...
           'Value must match regular expression {String}', rs.hid
...
INSERT INTO rule_type
    ...
    SELECT '2D531048-A876-4B5D-8D21-54074910C721', 'CharacterLimit',
           ...
           'Value must contain at most {Number} characters.', rs.hid
    FROM rule_subtype rs WHERE rs.name = 'String';
